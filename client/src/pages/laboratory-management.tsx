  ...
  const handlePrintReport = async (test: any) => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `Lab-Report-${test.patient?.firstName || 'Patient'}-${test.patient?.lastName || 'Report'}-${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      toast({
        title: "Print Failed",
        description: "Unable to generate the laboratory report for printing.",
        variant: "destructive",
      });
    }
  };
                              ...
                              onClick={() => {
                                ...
                                if (test.parameterResults) {
                                  try {
                                    ...
                                    // Convert structured results back to form values
                                    const formValues: { [key: string]: string } = {};
                                    savedResults.forEach((result: any) => {
                                      if (result.parameterId && result.value) {
                                        formValues[result.parameterId] = result.value;
                                      }
                                    });
                                    setResultValues(formValues);
                                  } catch (error) {
